...
class GSpreadService:
    ...
    def ready(self):
        ...
        g_auth = GOOGLE_JSON_AUTH
        if EV_G_JSON_AUTH is not None:
            print('Use G-Auth environ---------------------------------------')
            g_atuh = EV_G_JSON_AUTH
        else:
            print('Use G-Auth config----------------------------------------')
        
        if type(g_auth) == str:
            print('G-Auth type : String -------------------------------------')
            g_auth = json.loads(g_auth)
        elif type(g_auth) == dict:
            print('G-Auth type : Dict ---------------------------------------')
            if 'auth' in g_auth:
                g_auth = json.loads(g_auth.auth)
            elif len(g_auth.keys()) == 1:
                g_auth = g_auth[g_auth.keys()[0]]
            else:
                print('Unknown G-Auth Key....-------------------------------')
        ...
        print(f'Use G-Spreadsheet url : {spreadsheet_url}--------------------')
